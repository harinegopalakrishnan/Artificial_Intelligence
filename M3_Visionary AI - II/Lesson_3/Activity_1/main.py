...
while True:
    ...
    # Define the minimum HSV values [Hue, Saturation, Brightness] for detecting skin color.
    lower_skin = np.array([0, 20, 70], dtype=np.uint8)
    # Define the maximum HSV values [Hue, Saturation, Brightness] for detecting skin color.
    upper_skin = np.array([20, 255, 255], dtype=np.uint8)

    # Create a mask to detect skin color
    # Create a binary mask by selecting pixels whose HSV values lie within the defined skin color range.
    # Create a binary mask that highlights skin-colored pixels and suppresses all other pixels.
